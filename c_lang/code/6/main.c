...
int respond_error(int socket_fd, int fd, const char *message) {
  ...
  char response[256]; // These error responses are small, e.g. "500 Internal
                      // Server Error"
...
int handle_req(char *request, int socket_fd) {
  ...
  // Content-Type header)
  {
    ...
    {
      char *path_ext =
          strrchr(path, '.'); // Could skip the strrchr() if done in to_path
      size_t ext_len = strlen(path_ext);

      // If the extension is less than 4 bytes (5, counting the '.'), set it to
      // NULL.
    ...
    while (bytes_to_write) {
      ...
      if (bytes_written == -1) {
        // If sending the 200 didn't succeed, the odds of 500 succeeding aren't
        // great!
        return respond_500(socket_fd, fd);
      }
